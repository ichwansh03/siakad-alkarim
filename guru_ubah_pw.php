...
?>
<div class="profile">
			<h3 style="font-size:18px;text-align:center;background:#1abc9c;color:#fff;padding:10px;margin:0">Ubah Password Guru</h3>
				<?php
					if($_SERVER['REQUEST_METHOD'] == "POST"){
						$tid = $_POST['nip'];
						$newpass  = $_POST['newpass'];
						$confirmpass  = $_POST['confirmpass'];
						if(empty($newpass) or empty($tid) or empty($confirmpass)){
							...
							$newpass = md5($newpass);
							$user->updatePasswordTeacher($tid, $newpass);
						}
					}
				?>
